...
uint256 hashGenesisBlock("0xe4b8e8641f0743174213171550c928989db3403bd0ec847418a3bbfc7e9317cf");
...
const string strMessageMagic = "SonCoin Signed Message:\n";
...
{
    // SonCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // SonCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		// Genesis block
        const char* pszTimestamp = "SonCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a5814813115273a129cff99907ba4a05d951873dae7acb6c973d0c9e7c88911a3dbc9aa600deac241b91707e7b4ffb30ad91c8e56e695a1ddf318592988afe0a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1395175788;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1956543;
	    ...
	    assert(block.hashMerkleRoot == uint256("0x904ecbdbf7ff7f4bbc7e4bea70522567f425aed62624113f62b4b0941dec170e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd2, 0xaa, 0xa1, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SonCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SonCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SonCoinMiner(CWallet *pwallet)
{
    printf("SonCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("SonCoin-miner");
    ...
    try { loop {
        ...
        printf("Running SonCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SonCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SonCoinMiner, pwallet));
}
