...
// Copyright (c) 2011-2012 SonCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SonCoinTEST0\r");
            Send(hSocket, "WHO #SonCoinTEST0\r");
        } else {
            // randomly join #SonCoin00-#SonCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #SonCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SonCoin%02d\r", channel_number).c_str());
        }
